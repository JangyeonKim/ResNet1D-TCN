...
def attpool(x):
    batch, channel, sx, sy = x.size()
    out = torch.zeros_like(x)

    att_mat = x
    classifi_mat = x
    
    sum_mat = torch.sum(att_mat, (2,3))
    sum_mat = sum_mat.view(batch, channel, 1, 1)
    prob_mat = (att_mat / sum_mat) * 0.00001

    out = torch.add(classifi_mat, prob_mat)
    return out
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        BT, C, H, W = x.size()
        #fold = int(C / 4)
        out = shift(x,n_segment=batch, fold_div=8,inplace=False)
        out = self.conv1(out)
        #print(1)
        out = self.bn1(out)
        #print(2)
