...
class Lipreading(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        if self.modality == 'video':
            ...
            x = self.trunk(x)   ## TSM_resnet -> B 추가
            #x = self.trunk(x)      ## nomal
            ...
            lengths = [_//320 for _ in lengths] # {160 : 0.01초, 320 : 0.02초, 640 : 0.04초}
    ...
    def forward(self, x):
        ...
        if self.modality == 'video':
            print(x.size())
            #x = x.view(32,1,29,88,88)
            B = 1
            # batch = 32 일때 lengths input
            ##lengths = [29, 28, 28, 26, 25, 24, 24, 24, 24, 23, 22, 22, 22, 20, 20, 20, 20, 19, 19, 18, 18, 18, 18, 18, 18, 16, 16, 15, 14, 14, 13, 6] 
            # batxh = 16 일때 lengths input
            #lengths = [29, 28, 25, 24, 23, 20, 20, 19, 19, 18, 18, 16, 16, 15, 14, 13] 
            lengths = [30]
            #B, C, T, H, W = x.size()
            x = self.frontend3D(x)
            print(x.size())
            Tnew = 30    # outpu should be B x C2 x Tnew x H x W

            #n_channels, s_time, sx, sy = x.shape
            x = x.transpose(1, 2)
            x = x.mean(2).reshape(-1,3,22,22)
            #x =  x.reshape(n_batch*s_time, n_channels, sx, sy)
       
            x = self.trunk(x)   ## resnet 이 부분!!!
            if self.backbone_type == 'shufflenet':
                x = x.view(-1, self.stage_out_channels)
            print(x.size())
            x = x.reshape(B, Tnew, x.size(1))
        elif self.modality == 'raw_audio':
            B, C, T = x.size()
            x = self.trunk(x)
            x = x.transpose(1, 2)
